...
---
# MySQL
[CentOS上使用yum安装MySQL8](https://blog.csdn.net/zp8126/article/details/137084854)

## 数据库的三范式：

1. 列的原子性不可分割成更多列
2. 非主键列要完全依赖于主键列
3. 非主键列之间不存在传递依赖。即非主键列不能依赖于其他非主键列
**范式的发展代表着数据库在结构逻辑上合理化的一个过程，范式的存在有助于减少数据冗余、避免更新异常、简化查询，并提高数据的完整性**

## 关于存储过程的（类比函数）：

本质是一组为了完成特定功能的SQL语句集，可以直接通过存储过程的名字进行使用,常用于存储数据处理和转换(增删改)、函数之类的SQL语句

关键词：《DELIMITER //》《procedure》

MySQL 中的触发器（Trigger）是一种特殊的存储过程，在创建包含多个语句的触发器时，一定要用到DELIMITER关键词

## 关于MySQL的ACID：

MySQL事务操作的四种特性即ACID：原子性、一致性、隔离性、持久性。
...
MySQL默认的join on关键字连接是内连接，要允许null出现改成左连接或右连接即可
...
**double float精度问题**——double64位，float32位，可能存在精度丢失取而代之的是**decimal**，它的底层是字符串类型，不存在丢失问题 

**MySQL范围的关键词**
关键词：between…and…/in/not exist/except/

**物理外键与逻辑外键**——前者是在建表时就添加，后者则是通过alter语句建表后添加，企业多用后者
